...
public class GerenciadorDeSessao {
	...
	private boolean horarioIsValido(Sessao sessaoExistente, Sessao sessaoAtual) {
		...
		if (ehAntes) {
			...
			return horarioAtual.plus(sessaoAtual.getFilme().getDuracao()).isBefore(horarioAtual);
	...
	public boolean cabe(Sessao sessaoAtual) {
		...
		return sessoesDaSala.stream().map(sessaoExistente -> horarioIsValido(sessaoExistente, sessaoAtual)).reduce(Boolean::logicalAnd).orElse(true);
